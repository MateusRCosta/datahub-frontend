...
import { UsuarioUpdateRequest, usuarioUpdateRequestInput, UsuarioUpdateRequestInput } from "../schema";
...
export function UsuarioEdita({ id }: UsuarioEditaProps) {
    ...
    const { isLoading, isError, error, data } = useRetornaUsuario({ enabled: open, id });

    const form = useForm<UsuarioUpdateRequestInput, unknown, UsuarioUpdateRequest>({
        mode: "onSubmit",
        resolver: zodResolver<UsuarioUpdateRequestInput, unknown, UsuarioUpdateRequest>(usuarioUpdateRequestInput),
        defaultValues: {
            nome: "",
            senha: "",
            admin: false,
    ...
    const onSubmit = async (formData: UsuarioUpdateRequest) => {
        ...
        if (response.status !== 204) {
            toast.error("Erro ao editar usuário, verifique os dados e tente novamente.");
        ...
        <DialogCustom
            ...
            descricao={
                <div className="flex w-full justify-between">
                    <span className="w-full">Gerencie os dados e permissões do usuário</span>
                    <div className="flex flex-1 w-full">
                        <HoverCard>
                            ...
                            <HoverCardContent className="flex flex-1 flex-col w-fit gap-2">
                                <div className="font-semibold text-lg">Informações do Registro</div>
                                <div className="flex-1 divide-y divide-border rounded-lg border">
                                    <div className="flex items-center justify-between gap-4 px-2 py-1">
                                        ...
                                        <span className="text-sm font-mono text-[10px]">{data?.data?.id}</span>
                                    </div>
                                    <div className="flex items-center justify-between gap-4 px-2 py-1">
                                        <span className="text-sm text-muted-foreground">E-mail:</span>
                                        <span className="text-sm font-medium">{data?.data?.email}</span>
                                    </div>
                                    <div className="flex items-center justify-between gap-2 px-2 py-1">
                                        <span className="text-sm text-muted-foreground">Criado em:</span>
                                        <span className="text-sm font-medium">
                                            {data?.data?.createdAt ? formatarData(data.data.createdAt) : "--"}
                                        </span>
                                    </div>
                                </div>
                            </HoverCardContent>
            ...
            open={open}
            setOpen={setOpen}
            trigger={<DialogTrigger asChild><PenBox className="mr-2 h-4 cursor-pointer hover:text-primary transition-colors" /></DialogTrigger>}
            isPending={isPending}
        >
            <FormProvider {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} id="form-edita-usuario" className="flex flex-col gap-2 h-full">
                        ...
                        {data && !isError && (
                            <>
                                <div className="space-y-4">
                                    ...
                                    <InputGenerico name="senha" label="Alterar Senha" type="password" placeholder="Deixe em branco para manter a atual" />
                                    <InputGenerico name="email" label="E-mail" type="email" value={data.data?.email} disabled />

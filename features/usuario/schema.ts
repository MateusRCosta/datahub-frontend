...
export const permissoesSchema = z.enum(['EDITAR_BASE_DADOS', 'EDITAR_CAMPANHAS', 'EDITAR_INTEGRACOES', 'VISUALIZAR_RELATORIOS']);
...
export const usuarioUpdateRequest = usuarioSchema.pick({
  nome: true,
  admin: true,
  permissoes: true,
}).extend({
...
export const usuarioUpdateRequestInput = usuarioUpdateRequest.omit({
  senha: true,
}).extend({
  senha: senhaValidator,
});

export type UsuarioUpdateRequestInput = z.input<typeof usuarioUpdateRequestInput>;

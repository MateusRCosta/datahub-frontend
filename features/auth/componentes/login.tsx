...
export default function Login() {
    ...
    const onSubmit = async (values: LoginFormData) => {
        ...
        if (response.status === 401) {
            ...
            form.setError("root", { message: "Sessão expirada, faça login novamente." });
            return;
        }

        if (response.status === 403) {
            toast.error("Usuário ou senha inválidos, verifique e tente novamente.");
            form.setError("root", { message: "Usuário ou senha inválidos, verifique e tente novamente." });
            return;
        }

        if (response.status === 400) {
            toast.error("Dados inválidos, verifique e tente novamente.");
            form.setError("root", { message: "Dados inválidos, verifique e tente novamente." });
            return;
        }
        ...
        form.setError("root", { message: "Erro interno, tente novamente mais tarde." });
        return;
    };
        ...
        <div className="mx-auto flex min-h-dvh min-w-dvw items-center justify-center">
            <Card className="w-[90%] rounded-lg px-4 py-6 shadow-xl transition-shadow duration-200 hover:shadow-sm sm:w-[80%] md:w-[60%] lg:w-[40%]">
                <CardHeader>
                    ...
                    <CardDescription>Acesse com as suas credenciais o sistema DataHub</CardDescription>
                </CardHeader>
                <CardContent>
                    <FormWrapper form={form} onSubmit={(data) => onSubmit(data)} id="login-form">
                        <FieldGroup>
                ...
                <CardFooter>
                    <Field orientation="horizontal">
                        <Button type="submit" form="login-form" size="lg" className="w-full" disabled={isPending}>

...
export function AlteraSenha({ onOpenChange, open = false }: alteraSenhaProps) {
    ...
    const onSubmit = async (senhas: AlteraSenha) => {
        ...
        if (response.status === 403) {
            ...
            form.setError("senha", { message: "Senha inválida, verifique e tente novamente." });
            return;
        }
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>Digite a sua senha antiga e a sua nova senha para poder alterar a senha</DialogDescription>
                </DialogHeader>
                <FormWrapper form={form} onSubmit={onSubmit} id="form-altera-senha">
                    <FieldGroup>
                        <InputGenerico name="senha" label="Senha atual" type="password" />
                        <InputGenerico name="novaSenha" label="Nova senha" type="password" />
                        <InputGenerico name="confirmaSenha" label="Confirma nova senha" type="password" />
                    </FieldGroup>
                    <FieldError>{form.formState.errors.root?.message}</FieldError>
                </FormWrapper>
                <DialogFooter>
                    <DialogClose asChild>
                        <Button variant="outline" disabled={isPending}>Sair</Button>
                    </DialogClose>

...
export function DialogDeleta({ id, path, nome, mensagens, objeto }: DialogDeletaProps) {
    const [open, setOpen] = useState(false);
    const { mutateAsync, isPending } = useDeleta({ path, id });

    const handleDeleta = async () => {
        const resultado = await mutateAsync({ id, path });
        if (resultado.statusCode === 400 && resultado.message.includes("padrao")) {
            toast.error(mensagens?.padrao ?? `Não pode ser alterado pois é um registro padrão.`)
            return;
        }
    ...
    return (
        <>
            <Dialog open={open} onOpenChange={setOpen}>
                ...
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>{objeto}: {nome}</DialogTitle>
                        <DialogDescription>Após a exclusão, não será possivel recuperar os dados.</DialogDescription>
                    </DialogHeader>
                    {mensagens?.confirmacao ?? <p>Tem certeza que deseja excluir <strong>{nome}</strong>?</p>}
                    <DialogFooter>
                        <div className="w-full h-full flex flex-row justify-between gap-2">
                            <Button variant={"destructive"} onClick={() => handleDeleta()} disabled={isPending}>Excluir</Button>
                            <DialogClose asChild>

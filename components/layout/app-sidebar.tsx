...
import { Sidebar, SidebarContent, SidebarGroup, SidebarGroupContent, SidebarMenu, SidebarMenuButton, SidebarMenuItem, SidebarFooter, SidebarRail, useSidebar, SidebarGroupLabel } from "@/components/ui/sidebar";
import useFazLogout from "@/features/auth/api/use-faz-logout";
import { Me } from "@/features/auth/componentes/me";
import { useAuth } from "@/features/auth/provider/auth-provider";
import { RESOURCE_CONFIG, ResourceEntry, ResourceName } from "@/features/auth/config/resources";
...
export function AppSidebar() {
    ...
    const menuItems: MenuItem[] = useMemo(() => {
        const items: MenuItem[] = [
            { id: "home", label: "Home", icon: House, visible: true, adminOnly: false },
        ];
    ...
    const menuPrincipal = menuItems.filter(item => item.visible && !item.adminOnly);
        ...
        <Sidebar collapsible="icon" className="border-0">
            <SidebarContent className="bg-primary">
                ...
                <SidebarGroup>
                    ...
                    {menuAdmin.length > 0 && (
                        <SidebarGroupContent>
                            <SidebarGroupLabel className="text-white font-medium">Administração</SidebarGroupLabel>
                            <SidebarMenu>
                                {menuAdmin.map((item) => {
                                    const Icon = item.icon;
                                    return (
            ...
            <SidebarFooter className="bg-primary border-t rounded-b-none border-white">
                <SidebarMenu>
                    <SidebarMenuItem>
                        <SidebarMenuButton onClick={handleAlteraTema} className="text-white">
                    ...
                    <SidebarMenuItem>
                        <SidebarMenuButton onClick={() => setOpen(!open)} className="text-white">
                    ...
                    <SidebarMenuItem>
                        <SidebarMenuButton onClick={handleLogout} className="text-vermelho" disabled={isPending}>
